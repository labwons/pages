from labwons.util import DP, web, multiframes
...
class urls:
    ...
    def __call__(self, assetType:str, page:str, ReportGB:str, stkGb:str) -> str:
    ...
    def gb(self) -> str:
        if not hasattr(self, '__gb__'):
            try:
                tbs = pd.read_html(io=self.snapshot, header=0, encoding='utf-8', displayed_only=False)
                if tbs[11].iloc[1].isnull().sum() > tbs[14].iloc[1].isnull().sum():
                    self.__setattr__('__gb__', "B")
                else:
                    self.__setattr__('__gb__', "D")
...
class FnGuide:
    ...
    def abstract(self) -> DataFrame:
        ...
        def _get_(index:int) -> DataFrame:
            ...
            data = data.head(len(data) - len([i for i in cols if i.endswith(')')]) + 1)
            data.index.name = '기말'
            data.index = [
                DP.delKeys(idx, "(E) : Estimate 컨센서스, 추정치 ", "(P) : Provisional 잠정실적 ") for idx in cols
            ]
    ...
    def cashFlow(self) -> DataFrame:
        ...
        def _get_(index:int) -> DataFrame:
            ...
            data.index = [DP.delKeys(x, *cut) for x in data.index]
            data = data.T
            if index == 5:
                data.index = [
    ...
    def consensusProfit(self) -> DataFrame:
        ...
        for y, q in zip(yy, qq):
            yy[y] = yy[y].apply(DP.str2num)
            qq[q] = qq[q].apply(DP.str2num)
    ...
    def multipleBand(self) -> DataFrame:
        ...
        def _get_(key: str) -> DataFrame:
            ...
            for col in data:
                data[col] = data[col].apply(DP.str2num)
            return data
        return pd.concat(objs={'PER': _get_('CHART_E'), 'PBR': _get_('CHART_B')}, axis=1)
    ...
    def multiplesOutstanding(self) -> Series:
        ...
        data = {
            "fiscalPE": DP.str2num(src[src.index('PER') + 1]),
            "forwardPE": DP.str2num(src[src.index('12M PER') + 1]),
            "sectorPE": DP.str2num(src[src.index('업종 PER') + 1]),
            "priceToBook": DP.str2num(src[src.index('PBR') + 1]),
            "dividendYield": DP.str2num(src[src.index('배당수익률') + 1]),
        }
        return Series(data)
    ...
    def shareHolders(self) -> Series:
        ...
        example     :
            ...
            dtype: float64
        """
        data = web.data(self.url.cdn.shares).replace("", np.nan)
    ...
    def shareInstitutes(self) -> Series:
        ...
        return data.replace("관련 데이터가 없습니다.", np.nan)
    ...
    def snapShot(self) -> Series:
        ...
        return Series({
            ...
            "previousClose": DP.str2num(src.find("close_val").text),
            "fiftyTwoWeekHigh": DP.str2num(src.find("high52week").text),
            "fiftyTwoWeekLow": DP.str2num(src.find("low52week").text),
            "marketCap": DP.str2num(src.find("mkt_cap_1").text),
            "sharesOutstanding": DP.str2num(src.find("listed_stock_1").text),
            "floatShares": DP.str2num(src.find("ff_sher").text),
            "volume": DP.str2num(src.find("deal_cnt").text),
            "foreignRate": DP.str2num(src.find("frgn_rate").text),
            "beta": DP.str2num(src.find("beta").text),
            "return1M": DP.str2num(src.find("change_1month").text),
            "return3M": DP.str2num(src.find("change_3month").text),
            "return6M": DP.str2num(src.find("change_6month").text),
            "return1Y": DP.str2num(src.find("change_12month").text),
            "return3Y": DP.str2num(src.find("change_36month").text),
        })
